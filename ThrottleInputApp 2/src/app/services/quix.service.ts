...
export class QuixService {
  ...
  public workspaceId: string = 'demo-simulinkcombustionengine-frontend'; // Look in the URL for the Quix Portal your workspace ID is after 'workspace='
  public topicName: string = 'throttle-input'; // get topic name from the Topics page in the Quix portal
  /* optional */
  /*~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-*/
  ...
  constructor(private httpClient: HttpClient) {
    ...
    if (this.workingLocally || location.hostname === "localhost" || location.hostname === "127.0.0.1") {
      ...
      let topic$ = this.httpClient.get(this.server + 'output_topic', { headers, responseType: 'text' });
